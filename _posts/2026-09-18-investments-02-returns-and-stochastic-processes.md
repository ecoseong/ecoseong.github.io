...
$$X\in\{0,1\}$$이고 $$\Pr(X=1)=p$$인 한 번의 시행은 베르누이분포다.

$$
\Pr(X=x)=p^x(1-p)^{1-x},\qquad x\in\{0,1\}.
$$
...
독립적인 베르누이 시행을 $$n$$번 더한 성공 횟수는 이항분포를 따른다.

$$
Y=\sum_{i=1}^{n}X_i\sim\operatorname{Binomial}(n,p),
\qquad \Pr(Y=k)=\binom nkp^k(1-p)^{n-k}.
$$

원문은 0과 1 두 값을 갖는 예를 이항분포라고 소개한다. 이는 시행 횟수가 1인 특수한 경우다. 원문은 $$\Pr(X=0)=p$$로 두었지만 이 글에서는 관례에 따라 $$\Pr(X=1)=p$$로 통일한다.
...
이 글은 정규분포 표기를 다음처럼 통일한다.

$$
\boxed{X\sim N(m,v)\quad\text{에서 두 번째 인수 }v\text{는 분산이다.}}
$$

따라서 평균이 $$\mu$$, 표준편차가 $$\sigma>0$$이면
...
표준정규 누적분포함수를 $$\Phi$$라고 할 때,

$$
\Pr(X\le c)=\Phi\left(\frac{c-\mu}{\sigma}\right),
$$
...
표준 위너 과정 $$W_t$$는 다음 성질을 갖는다.

1. \$$W_0=0$$.
2. 겹치지 않는 시간구간의 증분은 독립이다.
3. \$$W_{t+h}-W_t\sim N(0,h)$$ for \$$h>0$$.
4. 경로가 거의 확실하게 연속이다.

작은 구간 $$\Delta t$$에서는

$$
\Delta W=\varepsilon\sqrt{\Delta t},\qquad \varepsilon\sim N(0,1)
$$
...
$$
\mathbb E[\Delta W]=0,
\quad \operatorname{Var}(\Delta W)=\Delta t,
\quad \operatorname{SD}(\Delta W)=\sqrt{\Delta t}.
$$
...
$$
W_T-W_0=\sum_{i=1}^{N}\varepsilon_i\sqrt{\Delta t}.
$$

기대값의 선형성으로

$$
\mathbb E[W_T-W_0]=\sum_{i=1}^{N}0=0.
$$
...
$$
\boxed{W_T-W_0\sim N(0,T),\qquad
\operatorname{SD}(W_T-W_0)=\sqrt T.}
$$
...
$$
\Pr(W_{t+h}\le x\mid\mathcal F_t)
=\Pr(W_{t+h}\le x\mid W_t).
$$
...
$$
\boxed{X_{t+h}-X_t\sim N(ah,b^2h)},
$$

$$
\mathbb E[X_{t+h}\mid\mathcal F_t]=X_t+ah,
\qquad \operatorname{SD}(X_{t+h}\mid\mathcal F_t)=|b|\sqrt h.
$$
...
드리프트만 존재하면 직선 $$X_t=X_0+at$$이고, 위너 항을 더하면 그 직선 주변에서 무작위로 움직인다. 장래의 값 자체는 정규분포이므로 음수가 될 가능성도 남는다.
...
따라서 상수 $$\mu,\sigma$$와 $$S_0>0$$에 대해 다음 모형을 도입한다.
...
$$\Delta t$$가 작고 구간 초 가격을 $$S_t$$로 고정하는 Euler 근사는

$$
\frac{\Delta S_t}{S_t}\approx\mu\Delta t+\sigma\sqrt{\Delta t}\,\varepsilon_t.
$$

따라서 조건부 근사분포는

$$
\boxed{\frac{\Delta S_t}{S_t}\ \Big|\ \mathcal F_t
\ \approx N(\mu\Delta t,\sigma^2\Delta t).}
$$
...
$$m=\mu-\sigma^2/2$$를 로그수익률의 연율 평균으로 정의하자.

| 대상 | 정확한 성질 |
|---|---|
| 로그수익률 $$\ell_{t,t+h}=\ln(S_{t+h}/S_t)$$ | $$N(mh,\sigma^2h)$$ |
| 총수익배율 $$S_{t+h}/S_t$$ | 로그정규분포 |
| 주가 $$S_{t+h}$$, 현재 가격이 주어졌을 때 | 로그정규분포, 항상 양수 |
| 단순수익률 $$R_{t,t+h}=S_{t+h}/S_t-1$$ | 로그정규변수에서 1을 뺀 분포, 하한은 -1 |
...
정규변수 $$Z\sim N(a,v)$$에 대해 $$\mathbb E[e^Z]=e^{a+v/2}$$이므로

$$
\mathbb E[S_{t+h}\mid\mathcal F_t]=S_te^{\mu h},
$$

$$
\mathbb E[R_{t,t+h}]=e^{\mu h}-1,
\qquad \mathbb E[\ell_{t,t+h}]=\left(\mu-\frac{\sigma^2}{2}\right)h.
$$
...
한 해를 252거래일로 놓고, 독립·동일분포인 로그증분을 가정하자. 연율 로그평균이 $$m$$, 연율 변동성이 $$\sigma$$이면

$$
\ell_{\mathrm{day}}\sim N\left(\frac m{252},\frac{\sigma^2}{252}\right).
$$

따라서

$$
\boxed{m_{\mathrm{day}}=\frac m{252},\qquad
\sigma_{\mathrm{day}}=\frac\sigma{\sqrt{252}}.}
$$

역으로 $$n$$기간의 독립 로그수익률을 합하면 평균은 $$n$$배, 표준편차는 $$\sqrt n$$배다.

$$
\mathbb E\left[\sum_{i=1}^n\ell_i\right]=nm_d,
\qquad
\operatorname{Var}\left(\sum_{i=1}^n\ell_i\right)=n\sigma_d^2.
$$

**주의:** 평균을 단순히 252배 하는 것은 연율 로그평균 또는 단순수익률 평균의 선형 연율화다. 실현 복리수익률은 $$\prod(1+R_i)-1$$로 계산해야 한다. 자기상관이 있으면 분산의 제곱근 환산도 수정해야 한다.
...
$$
R_d\sim N\left(\frac{0.12}{252},\frac{0.15^2}{252}\right).
$$
...
\begin{aligned}
\Pr(R_d\le-0.23)
&=\Phi\left(\frac{-0.23-0.00047619}{0.00944911}\right)\\
&=\Phi(-24.3913)\\
...
$$
\ell_d\le\ln(0.77)\approx-0.261365
$$
...
\qquad \ell_t=\ln(1+R_t)
$$

처럼 변환한다. 배당·분할을 일관되게 조정한 총수익지수를 이용할 수도 있다.
...
히스토그램을 위해 $$r_{\min}<r_{\max}$$인 관측범위를 $$K$$등분하면 구간 폭은

$$
\Delta x=\frac{r_{\max}-r_{\min}}K.
$$
...
모집단 평균은 $$\mu_r$$, 표본평균은 $$\bar r$$로 구분한다. 표본평균은 자료가 바뀌면 값도 바뀌는 확률변수다.

### 12.2 분산: 분모가 N인 경우와 N-1인 경우

경험분포 자체의 분산은

$$
\widehat v_N=\frac1N\sum_{i=1}^{N}(r_i-\bar r)^2.
$$

i.i.d. 표본에서 모집단 분산의 불편추정량은 $$N>1$$일 때

$$
\boxed{s^2=\frac1{N-1}\sum_{i=1}^{N}(r_i-\bar r)^2.}
$$

둘은

$$
\widehat v_N=\frac{N-1}{N}s^2
$$

의 관계를 갖는다. 원문의 $$1/N$$ 식은 경험분산으로는 맞다. 이를 불편표본분산과 혼동하지 않는 것이 중요하다. $$N-1$$이 필요한 이유는 [부록 D](#appendix-d)에서 증명한다.
...
$$
\widehat m_k=\frac1N\sum_{i=1}^{N}(r_i-\bar r)^k
$$

라고 정의하자. $$\widehat m_2>0$$일 때 적률형 표본왜도와 표본첨도는

$$
\boxed{\widehat\gamma_1=\frac{\widehat m_3}{\widehat m_2^{3/2}}},
\qquad
\boxed{\widehat\beta_2=\frac{\widehat m_4}{\widehat m_2^2}}.
$$

초과첨도는

$$
\widehat\gamma_2=\widehat\beta_2-3.
$$
...
첨도를 단지 “봉우리의 높이”로 해석하면 부정확하다. 또한 왜도 0이 반드시 대칭성을 뜻하지 않고, 첨도 3이 정규분포를 보장하지도 않는다. 위 표본적률 비율은 유한표본 불편보정을 적용한 통계량과는 다르다.

## 13. 표준오차: 추정한 평균은 얼마나 정확한가?

$$r_i$$가 i.i.d.이고 $$\mathbb E[r_i]=\mu_r$$, $$\operatorname{Var}(r_i)=\sigma_r^2<\infty$$라고 하자.

기대값은

$$
\mathbb E[\bar r]=\frac1N\sum_i\mathbb E[r_i]=\mu_r.
$$
...
$$
\boxed{\operatorname{SE}(\bar r)=\frac{\sigma_r}{\sqrt N},
\qquad \widehat{\operatorname{SE}}(\bar r)=\frac{s}{\sqrt N}.}
$$
...
$$N=252$$, $$\bar r=0.0005$$, $$s=0.01$$이면

$$
\widehat{\operatorname{SE}}(\bar r)=\frac{0.01}{\sqrt{252}}
\approx0.00062994.
$$
...
고정된 기간 $$T$$년의 GBM을 $$N$$등분해 로그수익률을 관측하면

$$
\widehat m=\frac1T\sum_{i=1}^{N}\ell_i
=\frac{\ln(S_T/S_0)}T,
\qquad \operatorname{SE}(\widehat m)=\frac\sigma{\sqrt T}.
$$
...
| 12쪽, 식 (29) | 정규분포의 두 번째 인수에 표준편차 사용 | 이 글은 분산 표기로 통일 |
| 13쪽 | 로그수익률 평균과 가격과정 드리프트에 같은 기호 사용 | $$m=\mu-\sigma^2/2$$로 구별 |
...
| 20쪽 | 히스토그램 구간 폭에 최댓값·최솟값의 합 | $$(r_{\max}-r_{\min})/K$$, 경계 중복 방지 |
...
- GBM의 정규 로그수익률과 로그정규 주가 관계는 [Columbia University의 GBM 강의노트](https://www.columbia.edu/~ks20/FE-Notes/4700-07-Notes-GBM.pdf)와 교차 확인했다. 그 자료의 로그 드리프트 표기는 이 글의 \$$m$$에 해당하므로 기호를 그대로 혼용하지 않았다.
...
에서 $$S_t$$의 명시적 표현을 얻고 싶다. $$f(S)=\ln S$$를 적용하면 곱셈적 변화를 덧셈적 변화로 바꿀 수 있다.
...
$$
df=\left(f_t+\mu S_tf_S+\frac12\sigma^2S_t^2f_{SS}\right)dt
+\sigma S_tf_S\,dW_t
$$
...
\begin{aligned}
d\ln S_t
&=\left[\frac1{S_t}\mu S_t
+\frac12\left(-\frac1{S_t^2}\right)\sigma^2S_t^2\right]dt
+\frac1{S_t}\sigma S_t\,dW_t\\
...
$$Z\sim N(a,v)$$이고 $$v>0$$이면

$$
\mathbb E[e^{uZ}]
=\frac1{\sqrt{2\pi v}}\int_{-\infty}^{\infty}
\exp\left[uz-\frac{(z-a)^2}{2v}\right]dz.
$$
...
$$
uz-\frac{(z-a)^2}{2v}
=-\frac{(z-a-uv)^2}{2v}+ua+\frac12u^2v.
$$
...
\int_{-\infty}^{\infty}\exp\left[-\frac{(z-a-uv)^2}{2v}\right]dz.
$$

뒤의 정규밀도 적분은 1이므로

$$
\boxed{\mathbb E[e^{uZ}]=e^{ua+u^2v/2}.}
$$
...
$$
Z\sim N\left(\left(\mu-\frac12\sigma^2\right)T,\sigma^2T\right).
$$
...
$$r_1,\ldots,r_N$$이 i.i.d.이고 평균 $$\mu_r$$, 유한한 분산 $$\sigma_r^2$$를 갖는다고 하자.

우선 항등식

$$
\boxed{\sum_{i=1}^{N}(r_i-\bar r)^2
=\sum_{i=1}^{N}(r_i-\mu_r)^2-N(\bar r-\mu_r)^2}
$$

을 보이자. $$r_i-\bar r=(r_i-\mu_r)-(\bar r-\mu_r)$$이므로

$$
\begin{aligned}
\sum_i(r_i-\bar r)^2
&=\sum_i(r_i-\mu_r)^2
-2(\bar r-\mu_r)\sum_i(r_i-\mu_r)
+N(\bar r-\mu_r)^2\\
&=\sum_i(r_i-\mu_r)^2
-2N(\bar r-\mu_r)^2+N(\bar r-\mu_r)^2.
\end{aligned}
$$

마지막 줄에서 $$\sum_i(r_i-\mu_r)=N(\bar r-\mu_r)$$를 사용했다.

양변의 기대값을 취하면

$$
\begin{aligned}
\mathbb E\left[\sum_i(r_i-\bar r)^2\right]
&=N\sigma_r^2-N\mathbb E[(\bar r-\mu_r)^2]\\
&=N\sigma_r^2-N\operatorname{Var}(\bar r)\\
&=N\sigma_r^2-\sigma_r^2\\
&=(N-1)\sigma_r^2.
\end{aligned}
$$
...
$$
\mathbb E\left[\frac1{N-1}\sum_i(r_i-\bar r)^2\right]=\sigma_r^2.
$$

분모를 $$N$$으로 나누면 기대값은 $$(N-1)\sigma_r^2/N$$이 되어 모집단 분산을 평균적으로 작게 추정한다. 반대로 모집단 평균을 이미 알아서 $$\bar r$$ 대신 $$\mu_r$$를 쓴다면 분모 $$N$$이 맞는다.
...
독립이며 분산이 $$\sigma_r^2$$로 같은 수익률 $$N$$개에 대해

$$
\operatorname{Var}\left(\frac1N\sum_ir_i\right)
=\frac1{N^2}\sum_i\sigma_r^2=\frac{\sigma_r^2}{N}.
$$
...
$$d\ln S_t=m\,dt+\sigma\,dW_t$$에서 총 관측기간을 $$T$$, 관측 간격을 $$h=T/N$$으로 두자. 각 로그수익률은

$$
\ell_i=mh+\sigma\sqrt h\,\varepsilon_i.
$$

연율 로그 드리프트의 추정량은

$$
\widehat m=\frac{\bar\ell}{h}
=\frac1{Nh}\sum_i\ell_i
=\frac1T\sum_i\ell_i.
$$
...
&=\frac1{T^2}\sum_i\operatorname{Var}(\ell_i)\\
...
$$
\boxed{\operatorname{SE}(\widehat m)=\frac\sigma{\sqrt T}.}
$$
...
이며 큰 $$N$$에서 $$\operatorname{Var}(\bar r)\approx\Omega/N$$이다. 실제 추정에서는 적절한 시차 절단과 가중을 사용한 HAC 추정 등으로 연결할 수 있다. 이 근사를 사용하려면 의존성에 관한 추가 조건이 필요하다.
...
$$
Q=\frac{(N-1)s^2}{\sigma_r^2}\sim\chi^2_{N-1}.
$$
...
&=\left(\frac{\sigma_r^2}{N-1}\right)^2\operatorname{Var}(Q)\\
&=\frac{\sigma_r^4}{(N-1)^2}2(N-1)\\
&=\frac{2\sigma_r^4}{N-1}.
\end{aligned}
$$
...
$$
\operatorname{SE}(s^2)=\sigma_r^2\sqrt{\frac2{N-1}},
\qquad
